...
var Sprite = (function () {
	...
	Sprite.prototype.animTransform = function (species, isCustomAnim) {
		...
		if (isCustomAnim) {
			...
			} else if (species.id === 'mimikyubusted' || species.id === 'mimikyubustedtotem') {
				// standard animation
			} else {
...
var Battle = (function () {
	...
	Battle.prototype.runMinor = function (args, kwargs, preempt, nextArgs, nextKwargs) {
		...
		while (minors.length) {
				...
				if (kwargs.silent && effect.id !== 'typechange' && effect.id !== 'typeadd') {
					...
					default:
						if (typeof kwargs.msg == "string") {
							// TPP: Added so the server can specify action text
							actions += kwargs.msg;
						} else {
				...
				if (kwargs.silent) {
					...
					default:
						if (effect.effectType === 'Move') {
							...
							if (typeof kwargs.msg == "string") {
								// TPP: Added so the server can specify text of these effects
								actions += kwargs.msg;
							} else {
			...
			case '-sidestart':
				...
				default:
					if (typeof kwargs.msg == "string") {
						//TPP: Added so the server can specify the text of these effects
						actions += kwargs.msg;
					} else {
	...
	*/
	...
	Battle.prototype.run = function (str, preempt) {
		...
		switch (args[0]) {
		case 'c': case 'c:': case 'chat': case 'bchat':

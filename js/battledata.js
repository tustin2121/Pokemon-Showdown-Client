...
var Tools = {
	...
	parseChatMessage: function (message, name, timestamp, isHighlighted) {
		...
		switch (cmd) {
		case 'me':
			if (!showMe) return '<div class="chat chatmessage-' + toId(nameid) + hlClass + mineClass + '">' + timestamp + '<strong style="' + color + '">' + clickableName + ':</strong> <em>/me' + Tools.parseMessage(' ' + target) + '</em></div>';
			return '<div class="chat chatmessage-' + toId(nameid) + hlClass + mineClass + '">' + timestamp + '<strong style="' + color + '">&bull;</strong> <em>' + clickableName + ' <i>' + Tools.parseMessage(' ' + target) + '</i></em></div>';
		case 'mee':
			if (!showMe) return '<div class="chat chatmessage-' + toId(nameid) + hlClass + mineClass + '">' + timestamp + '<strong style="' + color + '">' + clickableName + ':</strong> <em>/me' + Tools.parseMessage(' ' + target).slice(1) + '</em></div>';
	...
	parseMessage: function (str) {
		...
		// Twitch emotes
		str = window.processEmoteRegExp(str);
		
		if (options.hidelinks) {
			str = str.replace(/<a[^>]*>/g, '<u>').replace(/<\/a>/g, '</u>');
		}
		
		// The following all got replaced by formatText() above. Add back what's desired.
		///////////////////////////////////////////////////////////////////////////////
		// // ``code``
		// str = str.replace(/\`\`([^< ](?:[^<`]*?[^< ])??)\`\`/g,

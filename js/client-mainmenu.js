	...
	var MainMenuRoom = this.MainMenuRoom = this.Room.extend({
		...
		initialize: function () {
			...
			if (app.down) {
				...
				buf += '<p><div style="text-align:center"><img width="96" height="96" src="//tppleague.me/sprites/bw/teddiursa.png" alt="" /></div> Bear with us as we freak out.</p>';
			...
			buf += '<p><button class="button mainmenu4" name="joinRoom" value="leaguebuilder">TPPLeague</button></p>';
			buf += '</div>';
			
			buf += '<div class="menugroup">';
			buf += '<p><button class="button mainmenu4 onlineonly disabled" name="joinRoom" value="battles">Watch a battle</button></p>';
		...
		addNews: function () {
			...
			$.ajax("/news-embed.php?news", {
				success: function(data, txstat, xhr) {
					if (xhr.status == 204) return; //do nothing
					var embed = $(".news-embed");
					if (embed.length == 0) {
						self.addPseudoPM({
							title: "News",
